...
class FileForce extends EventEmitter {
    ...
    addMime(data, ownerKeyPair, destinationPublic, cb) {
        const peek = require('buffer-peek-stream');
        const fileType = require('file-type');

        let readstream = isStream(data) ? data : fs.createReadStream(data);


        peek(readstream, 6100, function (err, chunk, outputStream) {
            if (err) throw err;
            let mimeType = fileType(chunk);
            ...
            libcrypto.encryptStream(originalStream, encryptedStream, onetimeKey, (encryptionParams) => {
                ...
                this.ipfs.add(tmpFilePath, (error, result) => {
                    if (!error) {
                        ...
                        let mHashArray = multihash.fromB58String2(ipfsHash); // it still include first 2 bytes for a schema
                        ...
                        libcrypto.encryptObjectForParty(
                            ...
                            (tagEncryptionResult) => {
                                ...
                                this.storeEcTag(ecTag, (storeError, storeResult) => {
                                    if (!storeError) {
                                        this.emit('IPFS#ADD#ECTAG', storeResult.hash, storeResult.ecTag);
                                        if (cb) {
